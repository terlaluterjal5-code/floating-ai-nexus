  ...
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        try { body = (await request.json()) as { conversationId?: string }; } catch { return jsonError(400, "invalid_body", "Invalid JSON"); }
        const conversationId = body.conversationId;
        if (!conversationId) return jsonError(400, "invalid_body", "conversationId required");

        const { data: conv } = await supabase.from("conversations").select("id, user_id").eq("id", conversationId).maybeSingle();
        if (!conv || conv.user_id !== userId) return jsonError(404, "not_found", "Conversation not found");
