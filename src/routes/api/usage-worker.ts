...
type Body = {
  conversationId?: string; endpoint: string; model?: string;
  promptTokens?: number; completionTokens?: number; totalTokens?: number;
  latencyMs?: number; costUsd?: number;
  status?: "ok" | "error" | "aborted" | "rate_limited"; error?: string;
};
    ...
    handlers: {
      POST: async ({ request }) => {
        ...
        await recordUsage(supabase, {
          userId, conversationId: body.conversationId ?? null, endpoint: body.endpoint, model: body.model,
          promptTokens: body.promptTokens, completionTokens: body.completionTokens, totalTokens: body.totalTokens,
          latencyMs: body.latencyMs, costUsd: body.costUsd, status: body.status, error: body.error ?? null,
        });
